...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match our_model ({self.img_size[0]}*{self.img_size[1]})."
...
class SwinUnet(nn.Module):
    ...
    def load_from(self, config):
        ...
        if pretrained_path is not None:
            ...
            if "our_model" not in pretrained_dict:
                ...
                # print(msg)
                return
            pretrained_dict = pretrained_dict['our_model']
            ...
            for k in list(full_dict.keys()):
                if k in model_dict:
                    if full_dict[k].shape != model_dict[k].shape:
                        print("delete:{};shape pretrain:{};shape our_model:{}".format(k, v.shape, model_dict[k].shape))
                        del full_dict[k]
...
if __name__ == '__main__':
    ...
    model = SwinUnet(num_classes=1,img_size=224).to('cuda')
    # out = our_model(img)
    # print(out.shape)
    ...
    model = SwinUnet(num_classes=1).to('cuda')
